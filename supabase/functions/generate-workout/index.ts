...
Return ONLY valid JSON:
{
  ...
  // If workout ended up with no exercises, throw an error - no silent fallbacks
  if (matchedExercises.length === 0) {
    console.error(`❌ ERROR: No valid exercises generated for ${workoutStyle} workout targeting ${targetMuscles.join(', ')}`);
    throw new Error(`Failed to generate ${workoutStyle} workout: No valid exercises found for ${targetMuscles.join(', ')}. Please try different settings.`);
  }
